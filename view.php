...
?>
    ...
    <script>
        function setupNavigation() {
            ...
            const mobileContainer = document.getElement                                        -nav-container');
                ...
                al                                                , i) => {
                    ...
                    const isCTA = low.includes("avanzamos");
                                                             me = isCTA ? 'nav-link                                                                    li.                                                    {el.id}" class="${className}"><span>${navL                                                                   nav.appendChild(li);                                                     (mobileContainer) {
                        const mLi = document.createElement('li');
                        mLi.classN                                                    
                        mLi.inner                                                el                                            ile-nav-link" onclick="toggleM                                            an>${navLabel}</span></a>`;
                        mobileContai                                        ild(mLi);
                    }
                })                                            catch (e) {
                console.error("                                        g up navigation", e);
            ...
            return tracked;
                                               function                                             () {
            const menu = d                                        ntById('mobileNav');
            const isOpen = menu.cla                                            'open');
            if (isO                                               menu.classList.remove('open');
                document.bod                                        overflow = '';
            } else {
                menu.classList.add('open');
                do                                            e.overflow = 'hidden';
            }
        }
                                            tions = [];
        window.addEventListener('DOMContentLoaded', (                                                                                                  injectTeamSection();
            } catch (e) {
                                             onsole.error("Error Injecting Team Section", e);
            }
            ...
            // Ensure icons are created after dynamic gener                                            onAttempts = 0;
            const initIcons = () => {
                 if (window.lucide && window.lucide.createIcons) {
                                                                          wi                                        ons();
                    } catch(e) {}
                } el                                        ttempts < 50) { // Try for                                        ds max
                    iconAttempts++;
  ...
  .                                                e(() => {
                    const scrollPos =                                                indow.innerHeight / 3);
                                                        ent = "";
                                                                                                                 lY < 50) {
                                                                                      } else {
                                                                    (el => {
                                                                fs                                                    ient                                                scr                                                              if (offsetTop <= scrollPos) {
                                                              rrent = el.id;
                                                                                  });
                                                                       document.querySelectorAll(".nav-link, .n                                                        
                        l.                                                    ");
                        const href = l.getAttribute("href");
                                                                            "#" + current) {
                                                             ss                                                    }                                                & l.id === 'nav-                                                                  l.classList.add(                                                               }
                    });
                    
                                                             ll = window.scroll                                                    onst height = document.documentElement.scroll                                                    ght;
                    if (height > 0) {
                                                               led = (winScroll / height) * 100;
                                                           ile                                                ");
                                                              fil                                        scrolled + "%";
                    }
                                                    isScrolling                                         
        ...
        // Modal logic                                         const BOT_TOKEN = '8201699988:AAHZ6UeItc1I6EkULkrV6mozxkLO80t7j58';
                                            t CHAT_ID = '7313439878';
                                              DOC_NAME = '<?php echo addslashes($proposal["client_name"]); ?>';

                                              on openModal(type) {
                                                   t.getElementById('modal-' + type).classList.add('open');
        }
        ...
        document.querySelectorAll('.modal-overlay').forEach(m =>                                               m.addEventListener('click', e => { if (e.target === m) m.classList.remove('open'); });
                                                  ...
                                                  elegram(text) {
                                        rn fetch(`ttps://api.telegram.org/bot${BOT_TOKEN                                        {
                methd: 'POST',
                headers: { 'Content-Type': 'application/json' },
                                                body: JSON.stringify({ chat_id: CH AT_ID, text, parse_mode: 'Markdown' })
            });
                                               async function apiCall(action, par                                                    const formData = new URLSearchParams();
                                         ormData.append('api_action', action);                                             (let k in param                                        nd(k, paras[k]);
            return fetch(window.location.href, {                                         method: 'POST',
                headers:                                         nt-Type': 'application/x-www-form-urlencoded' },
                body: formData
            });
        }

        asy                                        submitApproval() {
                                          nst btn = document.querySelector(                                        rm .btn-modal-primary');
                                             isabled = true;
            btn.textContent = 'Enviando...';
            await apiCall('approve_doc');
            await sendTelegram(`✅ *Documento Aprobado*\nCliente: *${DOC_NAME}*\n\nEl cliente ha aprobado el documento funcional. Preparar presup                                        ;
            setTimeout(() => window.location.reload(), 1000); // Reload to show waiting state
        }

                                             ction submitomment() {
            const text = document.getElementById                                        xt').value.trim();
            if (!text) { document.getElementById('com                                        focus(); return; }
                                          nst btn = document.querySelector(                                        rm .btn-modal-primary');
           btn.disabled                                                 btn.textContent = 'Enviando...';
            await apiCall('comment_doc', { co                                        });
            await sendTelegram(`💬 *Comentarios del cliente*                                        ${DOC_NAME}*\n\n${text}`);
            document.getElementById('comment-form').                                        splay = 'none';
            document.getElementById('comment-success').style.display = 'block';
        }

        async func                                        dfApproval() {
                                              btn = document.querySelector('#ap                                        rm .btn-modal-primary');
                                             isabled = true;
            btn.textContent = 'Enviando...';
            await apiCall('approve_pdf');
            await sendTelegram(`✅💰 *Presupuesto Aprobado*\nCliente: *${DOC_NAME}*\n\n¡El clie                                        probado el presupuesto final!`);
            setTimeout(() => window.location.reload(), 1000); 
        }

        async                                        bmitPdfRejecion() {
            const text = document.getElementById('reje                                        ).value.trim();
            if (!text) { document.getElementById('reject-pd                                        us(); return; }
                                              btn = document.querySelector('#r                                        rm .btn-modal-primary');
           btn.disable                                                  btn.textContent = 'Enviando...';
            await apiCall('reject_pdf', { comment: text });
                                           it sendTelegram(`❌🧾 *Presupuesto Rechazado/Cambios*\nCliente: *${D                                        nComentario: ${text}`);
            document.getElementById('reject-pdf                                        e.display = 'none';
            document.getElement('reject-pdf-success').ste.display = 'block';
